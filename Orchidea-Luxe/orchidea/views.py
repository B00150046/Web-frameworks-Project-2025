...
class LoginCustomerView(generic.View):
    ...
    def post(self, request):
        ...
        try:
            logger.debug(f"Attempting login for customer with email: {email}")
            user = User.objects.get(email=email)
            if not check_password(password, user.password):
                logger.debug(f"Password mismatch for customer email: {email}")
            ...
            logger.debug(f"Login successful for customer email: {email}")
            return redirect("orchidea:homeCustomer", User_Id=user.id)
        except User.DoesNotExist:
            logger.debug(f"Customer user not found for email: {email}")
            return render(request, self.template_name, {"error": "Invalid email or password"})
        except Customer.DoesNotExist:
            logger.debug(f"No customer account linked to email: {email}")
...
class LoginEmployeeView(generic.View):
    ...
    def post(self, request):
        ...
        try:
            ...
            if not check_password(password, user.password):  # Validate the password
                logger.warning(f"Password mismatch for employee email: {email}")
                return render(request, self.template_name, {"error": "Invalid email or password"})
            
            # Fetch the associated Employee object
...
class SingleBookingViewDetail(generic.View):
    ...
    def get(self, request, User_Id, pk):
        ...
        customer = Customer.objects.get(user_ID=user)
        booking = get_object_or_404(Booking, id=pk)
        return render(request, self.template_name, {'user': user, 'customer': customer, 'booking': booking, 'user_type': 'customer'})
...
class AcceptRecruitmentView(generic.View):
    ...
    def post(self, request, User_Id, pk):
        ...
        # Update the status of the accepted form
        recruitment_form.status = 'Accepted'
        recruitment_form.save()

        # Delete all other recruitment forms for the same candidate
...
class RejectRecruitmentView(generic.View):
    ...
    def post(self, request, User_Id, pk):
        recruitment_form = get_object_or_404(RecruitmentForm, id=pk)
        recruitment_form.delete()  # Delete the selected recruitment form
        return redirect('orchidea:employee-recruitment', User_Id=User_Id)
...
class LoginCandidateView(generic.View):
    ...
    def get(self, request):
        # Render the candidate login page
    ...
    def post(self, request):
        ...
        try:
            ...
            # Set session variables for the logged-in candidate
            request.session["candidate_id"] = candidate.id
            request.session["user_name"] = user.username

            # Redirect to the candidate's home page
            return redirect("orchidea:homeCandidate", User_Id=user.id)
        except User.DoesNotExist:
            logger.error(f"User not found for email: {email}")
            return render(request, self.template_name, {"error": "Invalid email or password"})
...
class HomeCandidateView(generic.View):
    ...
    def get(self, request, User_Id):
        ...
        return render(request, self.template_name, {
            'candidate': candidate,
            'user_type': 'candidate',
            'User_Id': User_Id  # Ensure User_Id is passed to the context
        })
...
class ApplyFormView(generic.View):
    ...
    def post(self, request, User_Id):
        user = get_object_or_404(User, id=User_Id)
        candidate = get_object_or_404(Candidate, user_ID=user)
        selected_skills = request.POST.getlist('skills[]')  # Correctly retrieve the list of selected skills
        ...
        # Create a new RecruitmentForm
        recruitment_form = RecruitmentForm.objects.create(candidate_ID=candidate)

        # Create CandidateSkill objects for selected skills
...
class CandidateRecruitmentView(generic.View):
    ...
    def get(self, request, User_Id):
        ...
        for form in recruitment_forms:
            skills = [candidate_skill.skill_ID for candidate_skill in form.candidate_skills.all()]  # Retrieve related skills
...
class SingleCandidateRecruitmentView(generic.View):
    ...
    def get(self, request, User_Id, pk):
        ...
        candidate = Candidate.objects.get(user_ID=user)
        recruitment_form = get_object_or_404(RecruitmentForm, id=pk)
        return render(request, self.template_name, {'candidate': candidate, 'recruitment_form': recruitment_form, 'user_type': 'candidate'})
